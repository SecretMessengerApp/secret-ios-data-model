...
static NSString *ZMLogTag = @"fetchMessage";
...
- (void)setNeedsToBeUpdatedFromBackend:(BOOL)needsToBeUpdatedFromBackend {
    [self willChangeValueForKey: NeedsToBeUpdatedFromBackendKey];
    [self setPrimitiveValue:[NSNumber numberWithBool:needsToBeUpdatedFromBackend] forKey:NeedsToBeUpdatedFromBackendKey];
    [self didChangeValueForKey:NeedsToBeUpdatedFromBackendKey];
}

- (BOOL)needsToBeUpdatedFromBackend {
    [self willAccessValueForKey:NeedsToBeUpdatedFromBackendKey];
    NSNumber *value = [self primitiveValueForKey: NeedsToBeUpdatedFromBackendKey];
    [self didAccessValueForKey:NeedsToBeUpdatedFromBackendKey];
    return value.boolValue;
}

- (void)setModifiedKeys:(NSSet *)modifiedKeys {
    [self willChangeValueForKey: ZMManagedObjectLocallyModifiedKeysKey];
    [self setPrimitiveValue: modifiedKeys forKey:ZMManagedObjectLocallyModifiedKeysKey];
    [self didChangeValueForKey:ZMManagedObjectLocallyModifiedKeysKey];
}

- (NSSet *)modifiedKeys {
    [self willAccessValueForKey:ZMManagedObjectLocallyModifiedKeysKey];
    NSSet *value = [self primitiveValueForKey: ZMManagedObjectLocallyModifiedKeysKey];
    [self didAccessValueForKey:ZMManagedObjectLocallyModifiedKeysKey];
    return  value;
}
