...
public enum ConversationListType {
    case archived,
         ...
         noDisturbItem,
         noDisturbs,
         includeUnreadMessageTops,
         excludeUnreadMessageTops,
         excludeTopsAndNotturbed,
         contacts,
         groups,
         favorites,
         folder(_ folder: LabelType)
}
...
extension ZMConversationListDirectory: ConversationDirectoryType {
    ...
    public func conversations(by type: ConversationListType) -> [ZMConversation] {
        ...
        case .tops:
            return topConversations as! [ZMConversation]
        case .noDisturbs:
            return doNotDisturbedConversations as! [ZMConversation]
        case .excludeUnreadMessageTops:
            return topExcludeUnreadMessageConversations as! [ZMConversation]
        case .includeUnreadMessageTops:
            return topIncludeUnreadMessageConversations as! [ZMConversation]
        case .excludeTopsAndNotturbed:
            return excludeTopAndNotDisturbedConversations as! [ZMConversation]
        case .topItem:
            return []
        case .noDisturbItem:
            return []
...
fileprivate class ConversationListObserverProxy: NSObject, ZMConversationListObserver, ZMConversationListReloadObserver, ZMConversationListFolderObserver  {
    ...
    func conversationListDidChange(_ changeInfo: ConversationListChangeInfo) {
        ...
        var reload = false
         if changeInfo.conversationList === directory.pendingConnectionConversations {
            updatedLists = [.pending]
        } else if changeInfo.conversationList === directory.topConversations {
            updatedLists = [.tops]
        } else if changeInfo.conversationList === directory.topIncludeUnreadMessageConversations {
            updatedLists = [.includeUnreadMessageTops, .excludeUnreadMessageTops]
        } else if changeInfo.conversationList === directory.topExcludeUnreadMessageConversations {
            updatedLists = [.includeUnreadMessageTops, .excludeUnreadMessageTops]
        } else if changeInfo.conversationList === directory.doNotDisturbedConversations {
            updatedLists = [.noDisturbs]
            reload = true
        } else if changeInfo.conversationList === directory.excludeTopAndNotDisturbedConversations {
            updatedLists = [.excludeTopsAndNotturbed]
        ...
        observer.conversationDirectoryDidChange(ConversationDirectoryChangeInfo(reloaded: reload, updatedLists: updatedLists, updatedFolders: false))
    }
